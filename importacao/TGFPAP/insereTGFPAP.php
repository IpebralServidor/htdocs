...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['excelFile']) && $_FILES['excelFile']['error'] == UPLOAD_ERR_OK) {
        ...
        $sqlExclusaoTemp = "DELETE FROM TEMP_INSERT_TGFPAP";
        $sqlExclusaoTemp = sqlsrv_query($conn, $sqlExclusaoTemp);

        if ($sqlExclusaoTemp === false) {
            die(print_r(sqlsrv_errors(), true));
        }

        // Varre as linhas a partir da linha 2 (linha 1 = cabeçalho promoção, linha 2 = cabeçalho colunas)
