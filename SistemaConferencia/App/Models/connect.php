...
class Connect
{
	...
	var $uid = "sankhya";
	var $pwd = "tecsis";
	var $databaseName = "SANKHYA_TESTE";
	var $SQL;
	var $teste;
	var $cont;
	...
	function login($username, $password)
	{
		...
		if ($teste != '') {
			...
			if (!strcmp($password, $this->senha)) {
				...
				header("Location: ../../SistemaConferencia/View/listaconferencia.php");
			} else {
				header("Location: ../login.php?alert=2");
			}
		} else {

			echo ("<script> alert('Usuário Inexistente'); window.location.href='../login.php?alert=1'; </script>");
